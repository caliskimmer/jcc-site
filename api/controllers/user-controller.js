...
module.exports = function() {
    ...
    UserController.listUsers = async function(req, res) {
        ...
        if (user.role === User.roleMap.USER) {
            return res.status(403).json({
               success: false,
               users: null,
               reason: 'user of role "user" does not have permission to view users'
            });
        }
        ...
        try {
            ...
            log.error(`An error occurred listing users from database => ${err}`);
            return res.json({
                success: false,
                users: null,
    ...
    UserController.viewUser = async function(req, res) {
        ...
        try {
            ...
            log.error(`An error occurred getting user from database => ${err}`);
            return res.json({
                success: false,
                reason: 'an internal error occurred'
            })
        ...
        if (user.role === User.roleMap.USER && user.username !== userToView.username) {
            return res.status(403).json({
                success: false,
                reason: 'user of role "user" does not have permission to view this user'
            });
        }

        if (user.role === User.roleMap.ADMIN && userToView.role === User.roleMap.SUPERADMIN) {
            return res.status(403).json({
                success: false,
                reason: 'user of role "admin" does not have permission to view this user'
            });
        }
    ...
    UserController.updateUser = async function(req, res) {
        ...
        if (user.role === User.roleMap.USER && user.username !== userToUpdate.username) {
            return res.status(403).json({
                success: false,
        ...
        if (user.role === User.roleMap.ADMIN && userToUpdate.role !== User.roleMap.USER) {
            return res.status(403).json({
                success: false,
                reason: 'user of role "admin" can only update users of role "user"'
            });
        }

        if (req.body.password && user.username !== userToUpdate.username) {
            return res.status(403).json({
                success: false,
        ...
        if (req.body.password) {
            userToUpdate.password = await UserService.generatePasswordHash(req.body.password);
        }
    ...
    UserController.deleteUser = async function(req, res) {
        ...
        if (user.role === User.roleMap.USER) {
            return res.status(403).json({
               success: false,
               reason: 'user of role "user" does not have permission to delete users'
            });
        }

        if (user.role === User.roleMap.ADMIN && userToDelete.role !== User.roleMap.USER) {
            return res.status(403).json({
               success: false,
               reason: 'user of role "admin" can only delete users of role "user"'
            });
        }
        ...
        try {
            ...
            log.error(`An error occurred deleting user from database => ${err}`);
            return res.json({
                success: false,
                reason: 'an internal error occurred'
            })
    ...
    UserController.createUser = async function(req, res) {
        ...
        if (user.role < User.roleMap.SUPERADMIN && user.role >= req.body.role) {
            return res.status(403).json({
               success: false,
               reason: `user does not have permission to create a user of role ${User.getRoleFromNumber(req.body.role)}`
            });
        }

        let newUser = new User({
            username: req.body.username,
            password: await UserService.generatePasswordHash(req.body.password),
            role:     req.body.role
        });
        ...
        try {
            ...
            log.error(`An error occurred checking for duplicate users => ${err}`);
            return res.json({
               success: false,
               reason: 'an internal error occurred'
            });
